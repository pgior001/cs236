...
//    question5(simbaSession)
  ...
  private def listPartitions(iter: Iterator[Row]): Iterator[Row] = {
    println(iter.toList)
    return iter
  }
...
//    simba.sql("SELECT max(timeRead), min(timeRead) from trajectory where where date_format(timeRead, 'u') <= 5").show()
  ...
  def question5(simba : SimbaSession) : Unit ={
    ...
    var solution = poi.distanceJoin(trajectories, Array("poix", "poiy"), Array("x", "y"), 100).groupBy("poix", "poiy").count().sort(desc("count")).limit(20)
    solution.show(20)
  }
